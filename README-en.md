...
- Helpers in `Por/`: the five files `Tuple`, `Omega1`, `Relation`, `Closure` and `Chain` adapt helpers and constructions of `lean/Pattern` of [koteitan/bms-elem-pattern](https://github.com/koteitan/bms-elem-pattern). That project is licensed under CC BY-SA 4.0 and has the same author as this repository. Whether these parts are also released under Apache-2.0 is the author's decision and is pending. Until then these parts are under CC BY-SA 4.0. Each of these files says so in its header.
