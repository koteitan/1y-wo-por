...
**定義（言語）.** **言語** は関係記号の集まりで、各記号に引数の数が決まっている。引数が $`n`$ 個の記号を **$`n`$ 引数の記号** と呼ぶ。このリポジトリでは関数記号と定数記号は使わない。
...
**定義（原子図式）.** 元の組 $`v_0, \ldots, v_{n-1}`$ の **原子図式** とは、それらを入れたすべての原子式の真偽の表である。これを $`\mathrm{diag}(v_0, \ldots, v_{n-1})`$ と書く。

**記法（ビット）.** 原子式 $`\varphi`$ の真偽を、真なら 1、偽なら 0 で表したものを $`\varphi`$ の **ビット** と呼び、$`[\varphi]`$ と書く。原子図式は、ビットを決まった順に並べたものである。
...
このリポジトリの言語は、$`\lt`$ と、3 引数の記号 $`\mathrm{Rel}_j`$ と、2 引数の記号 $`\mathrm{Top}_j`$（$`j \in \mathbb N`$）からなる。記号の解釈は [07](07-relation-r.md) §2 で定める。ここでは次のことだけを使う。

- 高さ $`\gamma`$ の構造では、$`\mathrm{Rel}_j`$ は点どうしの関係である。$`\mathrm{Rel}_j`$ を **内部の関係** と呼ぶ。
- 高さ $`\gamma`$ の構造では、$`\mathrm{Top}_j`$ は、点から、領域の外にある $`\gamma`$ への関係である。この $`\gamma`$ を **上端** と呼び、$`\mathrm{Top}_j`$ を **上端述語** と呼ぶ。上端述語の解釈は、高さ $`\gamma`$ ごとに違う。

§3 から、量化子の無い部分は原子図式の集合で書き直せる。そこで $`\Sigma_1`$ 論理式を次の 5 つ組で表す。$`m, n, \mathit{bb}, r`$ は自然数である（[notes/01-design.md](../notes/01-design.md) §3.2）。

| 成分 | 意味 |
|---|---|
| $`m`$ | 記号の上限。$`\mathrm{Rel}_j`$ と $`\mathrm{Top}_j`$ は $`j \lt m`$ だけを使う |
| $`n`$ | 原子図式が読む変数の数。$`n \le r + \mathit{bb}`$ |
| $`D`$ | 完全な原子図式（下で定義する）の集合。量化子の無い部分を表す。$`D`$ を論理式の **行列** と呼ぶ |
| $`\mathit{bb}`$ | 存在量化する変数の数 |
| $`r`$ | パラメータの数 |

- $`n`$ 点の **完全な原子図式** は、$`\lt`$ のビット、3 引数の $`\mathrm{Rel}_j`$ のビット、2 引数の $`\mathrm{Top}_j`$ のビット（$`j \lt m`$）からなる。$`m, n`$ を決めると、完全な原子図式は有限個しかない。
- 点の列 $`v`$ の原子図式を $`\mathrm{diag}(v)`$ と書く（§3）。読まないビットを偽と読む場合は §8 で扱う。
- 変数の列は、パラメータ $`p_0, \ldots, p_{r-1}`$ のあとに証人 $`y_0, \ldots, y_{\mathit{bb}-1}`$ を並べたものである。行列はこの列の最初の $`n`$ 個を読む。
...
**例.** $`\exists y\ (p_0 \lt y)`$ は $`m = 0`$、$`n = 2`$、$`\mathit{bb} = 1`$、$`r = 1`$ で、$`D`$ は「ビット $`[v_0 \lt v_1]`$ が 1」である完全な原子図式の集合である（$`v_0 = p_0`$、$`v_1 = y`$）。高さ $`M`$ で真であることは、$`p_0 + 1 \lt M`$ と同じである。

## 8. 2 つの構造の比べ方と、見えるビット

このリポジトリの比べ方には、教科書の定義と違う点が 2 つある。

**違い 1：同じ記号を別に解釈する.** 関係 $`R`$ の定義（[07](07-relation-r.md) §4）では、高さ $`a`$ の構造と高さ $`b`$ の構造（$`a \lt b`$ は順序数）を比べる。上端述語 $`\mathrm{Top}_j`$ は、高さ $`a`$ では「$`a`$ への関係」、高さ $`b`$ では「$`b`$ への関係」と解釈する。したがって、そのままでは部分構造ではない。
...
**違い 2：見えるビット.** §7 の変数の列の中での番号を **位置** と呼ぶ。位置 $`s`$ の変数の値を $`v_s`$ と書く。論理式ごとに、どの上端述語のビットを読めるかを、条件 $`\mathrm{allow}(j, s)`$（$`j, s \in \mathbb N`$）で決める。$`\mathrm{allow}(j, s)`$ は「位置 $`s`$ の点を第 1 引数とする $`\mathrm{Top}_j`$ のビット $`[\mathrm{Top}_j(v_s, \cdot)]`$ を読める」ことを表す。読めるビットを **見えるビット**、読めないビットを **見えないビット** と呼ぶ。見えないビットは偽と読む。

| 条件 | 見えるビット | 使う場所 |
|---|---|---|
| いつも真 | すべて | 高さ $`\omega_1`$ の構造（[08](08-closure-chain.md) §1 で定義する Good） |
| $`\mathrm{allow}_{k,S}(j, s) :\iff j \lt k \lor (j = k \land s \in S)`$ | $`j \lt k`$ の $`\mathrm{Top}_j`$ と、$`j = k`$ で第 1 引数の位置が $`S`$ に入る $`\mathrm{Top}_k`$ | 段 $`(k, \eta)`$ の構造（[07](07-relation-r.md) §3 で定義する） |

$`\mathrm{allow}_{k,S}`$ の $`k \in \mathbb N`$ は層の番号、$`S \subseteq \mathbb N`$ は位置の集合である。層、段 $`(k, \eta)`$、$`S`$ の使い方は [07](07-relation-r.md) §3 で述べる。

$`\mathrm{allow}_{k,S}`$ での「見える」は、変数の位置だけで決まり、点の値に依らない。そのため次の 2 つの補題が成り立つ。

- **補題 1（同じビットなら同じ真偽）.** 2 つの解釈が、見えるビットで一致すれば、原子図式も真偽も一致する。
- **補題 2（見えないビットを偽にする）.** 見えないビットを偽にする操作は、完全な原子図式の上の関数 $`\mathrm{mask}_{\mathrm{allow}}`$ である。したがって「見えるビットだけを読む、行列 $`D`$ の論理式」は「すべてを読む、行列 $`\mathrm{mask}_{\mathrm{allow}}^{-1}(D)`$ の論理式」と同じ真偽を持つ。

補題 2 は、段 $`(k, \eta)`$ の論理式（[07](07-relation-r.md) §3）を、すべての記号を使う言語の論理式に訳す。[08](08-closure-chain.md) と [09](09-obligations.md) で、$`\omega_1`$ の構造との比較に使う。

## 9. このリポジトリでの使われ方

| 場所 | 使い方 |
|---|---|
| [README](../README.md)「関係 R」 | $`\preccurlyeq_{\Sigma_1}`$ と構造 $`\mathfrak A^{\gamma}_{k,\eta}`$（[07](07-relation-r.md) §3 で定義する） |
