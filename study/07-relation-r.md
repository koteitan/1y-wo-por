...
**理由.** 量化子の無い論理式 $`\mathrm{Top}_j(p_0, p_1)`$ を、パラメータ $`(\xi, x)`$ で使う（$`j = k`$ なら位置 0 を $`S`$ に入れる）。高さ $`a`$ では左辺、高さ $`b`$ では右辺を意味する。この性質は Lean の定理としては書いていない。証明では、似た形の `top_abs`（Good な点と $`\omega_1`$ の間での上端述語の一致、[09](09-obligations.md) §4）を使う。
...
どちらも $`\mathrm{Elem}`$ に反する。この性質は Lean では示していない。組合せの層も使わない。
