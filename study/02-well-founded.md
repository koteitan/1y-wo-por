...
1-Y の展開は辞書式順序を下げる（[05](05-1y-mountain.md) §7）。それでも辞書式順序だけでは停止は出ない。そこでラベルを使う（§6）。
...
規則 $`G`$ が読んでよいのは、$`t' \prec t`$ となる鍵 $`t'`$ での値 $`F(t')`$ だけである。以下、この「小さい鍵での値」を $`\mathrm{IH}(t')`$ と書く。$`t' \prec t`$ でない鍵では $`\mathrm{IH}(t')`$ は定義されない。

## 5. ガードつきの再帰

関係 $`R`$ の定義（[07](07-relation-r.md)）では、「どの鍵を読むか」が論理式（[03](03-sigma1-elementary.md) §2 で定義する）の中の変数の値で決まる。書く前に、読む鍵が小さいとは言えない。そこで次の形にする。

1. 読みたい値を「鍵が小さい $`\land`$ $`\mathrm{IH}(\text{鍵})`$」と書く。前半の条件を **ガード** と呼ぶ。鍵が小さくないところでは、この式は偽になる。
...
[07 関係 R](07-relation-r.md) では、1 が §5 の段の解釈（[07](07-relation-r.md) §5 で定義する）、2 が §5 のガードつきの等式、3 が §6 の補題（ガードを外す）と定理（定義の式）である。
...
状態の集合 $`X`$ の上の関係 $`\to`$ を考える。$`s \to t`$ は「状態 $`s`$ から 1 回で状態 $`t`$ に移る」と読む。$`\to`$ が整礎であることを、整礎な順序 $`(L, \lt)`$ を使って示す。$`L`$ の元を **ラベル** と呼ぶ。

**定理（ラベルによる停止）.** 状態とラベルの間の関係 $`\mathrm{valid}(s, a)`$ が次を満たすとする。
...
**証明.** $`a`$ についての整礎帰納法で、「$`\mathrm{valid}(s, a)`$ なら $`s`$ は到達可能」を示す。$`s \to t`$ なら $`t`$ に $`b \lt a`$ のラベルがあるので、帰納法の仮定から $`t`$ は到達可能である。$`\square`$

大事な点は、ラベルが 1 つの状態に 1 つに決まっている必要が無いことである。「どれか 1 つのラベルが付けられる」ことと「1 回移ると、もっと小さいラベルが付けられる」ことだけを使う。

1-Y の証明では次のように当てはめる（[06](06-combinatorial-layer.md)）。表の言葉のうち、式と展開 $`s[N]`$ は [05](05-1y-mountain.md) で、図式、表現、末尾のラベルは [06](06-combinatorial-layer.md) §1、§2、§6 で定義する。
